...
def main(rank: int, world_size: int, config):
    ...
    # We are using Gray Scale Transform here on RGB Images, rest other transforms are model speciic
    crop_size = 224
    resize_size = 256
    ...
    gray_scale_transform = ImageClassification(crop_size, resize_size, mean, std, interpolation)

    # Use ImageFolder to create dataset(s)
    train_dataset = ImageFolder(config.train_dir, transform=gray_scale_transform)
    val_dataset = ImageFolder(config.val_dir, transform=gray_scale_transform)
    test_dataset = ImageFolder(config.test_dir, transform=gray_scale_transform)
