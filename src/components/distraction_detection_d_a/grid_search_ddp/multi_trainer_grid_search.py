...
class Trainer:
    def __init__(self,
        ...
        self.optimizer = self.configure_optimizer(optimizer_choice, lr, end_lr, momentum, weight_decay)
    ...
    def configure_optimizer(self, optimizer_choice, initial_lr, end_lr, momentum, weight_decay):
    ...
    def configure_scheduler(self, scheduler_choice, initial_lr, end_lr):
        if scheduler_choice.lower() == 'cosineannealingwarmrestarts':
            # start with a 20-epoch interval for the first restart and double the length of each interval thereafter
            lr_scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(self.optimizer, T_0=5, T_mult=2, eta_min=0.0001)  # setting eta_min to a small fraction of initial_lr
            ...
            lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=self.num_epochs)
            ...
            # total number of epochs expected for training (Time Based Decay)
            total_epochs = self.num_epochs - 1
            ...
            lr_lambda = lambda epoch: (1 - float(epoch) / float(total_epochs)) + (float(epoch) / float(total_epochs))* end_lr * (1/initial_lr)
            lr_scheduler = optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=lr_lambda)
        elif scheduler_choice.lower() == 'multiplicativedecay':
            # lr(t)=initial_lr × (decay_rate**epoch)
            decay_rate = 0.95  # Adjust this decay rate as needed
            ...
            lr_scheduler = optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=lr_lambda)
        elif scheduler_choice.lower() == 'exponentiallr':
            gamma = 0.95  # decay factor
            lr_scheduler = optim.lr_scheduler.ExponentialLR(self.optimizer, gamma=gamma)
        elif scheduler_choice.lower() == 'manualpolynomialdecay':
            # lr= initial_lr × (1− (epoch/max_epoch))**power
            max_epoch = self.num_epochs  # Total number of epochs
            power = 1.0  # The power of the polynomial. Often set to 1.0 or 2.0
            lr_lambda = lambda epoch: (1 - float(epoch) / float(max_epoch)) ** power
            lr_scheduler = optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=lr_lambda)
        elif scheduler_choice.lower()== 'polynomiallr':
            max_epoch = self.num_epochs
            lr_scheduler = optim.lr_scheduler.PolynomialLR(self.optimizer, total_iters=max_epoch, power = 1.0)
        else:
            raise ValueError("Invalid scheduler choice. Choose among 'cosineannealingwarmrestarts', 'cosineannealinglr', 'lambdalr', 'linearinterpolationlr', 'multiplicativedecay','exponentialdecayexp', 'exponentiallr'.")
        return lr_scheduler
...
if __name__ == "__main__":
    ...
    print(f"End Learning Rate (end_lr): {config.end_lr}")
    world_size = 2
