...
class Trainer:
    ...
    def configure_scheduler(self, scheduler_choice, initial_lr, end_lr):
        if scheduler_choice.lower() == 'cosineannealinglr':
            ...
            lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=100)
            ...
            lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=10, eta_min=0)
        elif scheduler_choice.lower() == 'cosineannealingwarmrestarts':
            # T_max should be (10) for 10 cosine cycles
            # After every 10th epoch the cycle starts again
            lr_scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(self.optimizer, T_0=10, T_mult=2, eta_min=0.0, last_epoch=-1)
...
def main(rank: int, world_size: int, config):
    ...
    log_dir = os.path.join(os.path.dirname(__file__), "experiments_grid_search", config.experiment_name, "runs")
